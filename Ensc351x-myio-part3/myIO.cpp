...
//			   : E. Kan
//			   :
// Version     : Oct. 30th, 2017 -- tcdrain for socketpairs.
...
public:
	...
	int reading(int des, void * buf, int n, int min, int time, int timeout)
	{
		...
		if (buffered >= min || pair == -1) {
			...
			// allow wcsReadcond to return -1 for error if pair == -1
			...
			cvRead.wait(condlk, [this] {return (buffered) >= 0;});
			bytesRead = wcsReadcond(des, buf, n, min, time, timeout);
			if (bytesRead == -1) {
				// an error occurred.
...
ssize_t myWrite(int des, const void* buf, size_t nbyte) {
	...
	// vectlk.unlock(); // should we unlock vectlk here ?
	return write(des, buf, nbyte);  // des is not from a pair of sockets.
